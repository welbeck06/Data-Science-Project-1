...
# Clean the data: Remove units from Mileage, Engine, and Power, and convert to numeric
...
# Convert Year to factor and set its levels to include all possible years
used_cars_data$Year <- factor(used_cars_data$Year)

# Ensure that the levels of Year are consistent across training and prediction
# Set levels explicitly to include all unique years in the dataset
unique_years <- sort(unique(used_cars_data$Year))
used_cars_data$Year <- factor(used_cars_data$Year, levels = unique_years)

# Build the linear regression model
model <- lm(Price ~ Year + Kilometers_Driven + Engine + Fuel_Type + Power + Mileage + Transmission, data = used_cars_data)
...
# View the first few rows with predicted prices
head(used_cars_data)
